...
<html lang="en">
	<head>
		...
		<style type="text/css">
			.demo-chat{width: 50%;margin: 80 auto 40 auto;}
		</style>
	...
	<body>
		...
		<!-- inline scripts related to this page -->
			<script src="/bootstrap/assets/chart/js/Chart.js"></script>
		<script type="text/javascript">
			
		var randomScalingFactor = function(){ return Math.round(Math.random()*100)};
		//加载数据
		var d = "${chartList[0].subject}";
		var data1 = {
		    labels: ["${chartList[0].subject}", "${chartList[1].subject}", "${chartList[2].subject}", "${chartList[3].subject}", "${chartList[4].subject}", "${chartList[5].subject}"],
		    datasets: [
		        {
		            fillColor: "#CCCCFF",
		            strokeColor: "rgba(220,220,220,1)",
		            label: "2018年",
		            data: ["${chartList[0].s_count}", "${chartList[1].s_count}", 
		                   "${chartList[2].s_count}", "${chartList[3].s_count}", 
		                   "${chartList[4].s_count}", "${chartList[5].s_count}"]
		        }
		    ]
		}
		...
		window.onload = function(){
			...
			window.myBar = new Chart(ctx).Bar(data1, {
				responsive : true
			});
		}
